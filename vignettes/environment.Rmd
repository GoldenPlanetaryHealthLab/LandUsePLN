...
vignette: >
  %\VignetteIndexEntry{Environment Set Up}
...
* had implicit system dependencies on a personal laptop (GDAL, R, etc.)
...
Some tools (e.g., newer Rust-based package managers, `rv`, `uvr`) require newer versions of system libraries such as `glibc`.
...
* Prefer tools that are compatible with older, stable Linux baselines, i.e. `renv`
...
* `conda` packages often lag behind CRAN/PyPI
